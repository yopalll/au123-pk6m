...
// to24h converts "10:00" + "AM"/"PM" to "HH:MM"
...
func parseOpeningHours(row []string) (string, string) {
	// row is 0-indexed; col 9 = index 8, col 55 = index 54
	type timeMins struct{ mins int }
	var times []int

	// We look at indices 8..54
	maxIdx := len(row) - 1
	for i := 8; i <= 54 && i <= maxIdx; i++ {
		cell := strings.TrimSpace(row[i])
		if timeRe.MatchString(cell) {
			// Next cell should be AM/PM
			if i+1 <= maxIdx {
...
func guessCategory(name string) string {
	...
	return "Body" // default for body-category Excel
}
...
func main() {
	...
	// Allow running from any dir: use script source location
	// We'll use relative path from cwd
	cwd, _ := os.Getwd()
	baseDir = filepath.Join(cwd, "..", "..")            // viygo-app/database
	// Actually scripts is inside database/scripts, so:
	// If run from database/scripts: base = database/
	// Detect: if cwd ends in "scripts", go up one
	...
	excelFile = filepath.Join(baseDir, "body_bodyallcategory_uk.xlsx")
	...
	for i, row := range rows {
		...
		// Pad row to at least 74 cols to avoid index out of bounds
		for len(row) < 74 {
			row = append(row, "")
		}
		...
		if desc == "" {
			desc = salonName + " is a professional beauty and wellness salon."
		}
		...
		// ── Services ──────────────────────────────────────────────
		// Primary: cols 6(idx5), 7(idx6), 8(idx7)
		...
		// Additional: (col65=idx64, col66=idx65, col64=idx63), (col68=idx67, col69=idx68, col67=idx66)
