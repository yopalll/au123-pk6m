<x-layouts.public title="Booking Saya">
<div class="max-w-3xl mx-auto px-6 py-10">

    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('akun.index') }}" class="text-[#4BA3CC] hover:underline text-sm">← Akun</a>
        <h1 class="text-2xl text-[#1B2D6B]">Booking Saya</h1>
    </div>

    {{-- Tabs --}}
    <div class="flex gap-1 border-b border-gray-200 mb-6" x-data="{ tab: '{{ request('tab','mendatang') }}' }">
        @foreach(['mendatang'=>'Mendatang','selesai'=>'Selesai','dibatalkan'=>'Dibatalkan'] as $key => $label)
            <button @click="tab = '{{ $key }}'"
                    :class="tab === '{{ $key }}' ? 'border-b-2 border-[#1B2D6B] text-[#1B2D6B] font-semibold' : 'text-gray-500 hover:text-gray-800'"
                    class="px-4 pb-3 text-sm transition-colors">{{ $label }}</button>
        @endforeach

        {{-- Upcoming tab content --}}
        <template x-if="false"><!-- Alpine controls visibility below --></template>
    </div>

    @forelse ($orders as $order)
        <div class="border border-gray-100 rounded-2xl overflow-hidden mb-4 hover:border-[#C5E1F0] transition-colors">
            <div class="flex items-center justify-between p-5 bg-gray-50 border-b border-gray-100">
                <div>
                    <span class="text-xs font-mono font-bold text-gray-500 bg-gray-200 px-2 py-0.5 rounded">{{ $order->kode_order }}</span>
                    <span class="ml-3 text-xs font-semibold px-2 py-0.5 rounded-full
                        {{ $order->status === 'success' ? 'bg-green-100 text-green-700' :
                           ($order->status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-600') }}">
                        {{ ucfirst($order->status) }}
                    </span>
                </div>
                <span class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($order->date_order)->isoFormat('D MMM Y') }}</span>
            </div>
            <div class="p-5 flex items-center gap-4">
                <div class="w-14 h-14 rounded-xl bg-[#E8F4FB] flex items-center justify-center text-2xl flex-shrink-0">✂️</div>
                <div class="flex-1 min-w-0">
                    <div class="font-semibold text-gray-900">{{ $order->salon->nama_salon }}</div>
                    <div class="text-sm text-gray-500">{{ $order->details->pluck('service.nama')->implode(', ') }}</div>
                    <div class="text-sm text-gray-400 mt-0.5">{{ $order->salon->kota?->nama }}</div>
                </div>
                <div class="text-right flex-shrink-0">
                    <div class="font-bold text-[#1B2D6B]">Rp {{ number_format($order->total_pembayaran, 0, ',', '.') }}</div>
                    <div class="flex gap-2 mt-2">
                        @if ($order->status === 'pending')
                            <form action="{{ route('booking.batal', $order->kode_order) }}" method="POST">
                                @csrf
                                <button type="submit" class="text-xs text-red-500 hover:underline" onclick="return confirm('Batalkan booking?')">Batalkan</button>
                            </form>
                        @endif
                        @if ($order->status === 'success')
                            <a href="{{ route('kategori.show', $order->salon->services->first()?->kategori?->slug ?? 'rambut') }}"
                               class="text-xs text-[#4BA3CC] hover:underline">Booking Lagi</a>
                        @endif
                    </div>
                </div>
            </div>
        </div>
    @empty
        <div class="py-16 text-center">
            <div class="text-5xl mb-4">📅</div>
            <p class="text-lg text-gray-500 mb-2">Belum ada booking.</p>
            <p class="text-sm text-gray-400 mb-6">Ayo temukan salon favoritmu dan lakukan booking pertamamu!</p>
            <a href="{{ route('cari') }}"
               class="px-6 py-3 bg-[#1B2D6B] text-white rounded-full font-semibold hover:bg-[#4BA3CC] transition-colors">
                Jelajahi Salon
            </a>
        </div>
    @endforelse

    <div class="mt-6">{{ $orders->links() }}</div>
</div>
</x-layouts.public>
